...
For any questions except the meaning of life, please contact us at:
...
def main():
    ...
    if rank == RANK_INDEX["data_manager"]:
        ...
        from data_manager_proc import run_data_manager

        run_data_manager(comm_world, rank, size, RANK_INDEX)

    elif rank == RANK_INDEX["convergence_monitor"]:

        from convergence_monitor_proc import run_convergence_monitor

        run_convergence_monitor(comm_world, rank, size, RANK_INDEX)

    elif rank == RANK_INDEX["policy_filter"]:

        from policy_filter_proc import run_policy_filter

        run_policy_filter(comm_world, rank, size, RANK_INDEX)

    elif rank == RANK_INDEX["agent_pool_manager"]:

        from handler_pool_manager_proc import run_handler_pool_manager

        run_handler_pool_manager(comm_world, rank, size, RANK_INDEX)

    elif rank >= RANK_INDEX["agent_handler"]:

        from agent_handler_proc import run_agent_handler

        run_agent_handler(comm_world, rank, size, RANK_INDEX)
